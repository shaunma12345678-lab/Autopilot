...
//
// WHAT THIS DOES NOT DO. Predict how a stock will react. "Significant to the
// business" and "will move the price" are different questions, and this
// system draws that line everywhere else (see AssetDetail.tsx's header
// comment on why there's no price-target line, and lib/investment-verdict.ts
// for the same rule applied to the synthesis pass). This judges scale and
// structure from what the filing actually discloses — dollar amounts,
// counterparties, scope — never sentiment or forecast.
...
const SIGNIFICANCE_SYSTEM = `You read a single SEC 8-K filing and judge how structurally significant the disclosed event is to the business. You are NOT predicting stock price reaction — that question is off-limits and you must never mention price, market reaction, or investor sentiment.
...
Return JSON:
{
  ...
  "reasoning": "1-2 sentences citing the specific dollar figure, scope, or disclosed detail behind the rating"
}`,
